...
{	
	...
	cvResizeWindow("Result",SCREENWIDTH, SCREENHEIGHT);
	cvMoveWindow("Result",1280,0);
		...
		Mat Result = cv::Mat::zeros(1280,700,CV_8UC3);

	    cv::bitwise_not(BBLImage, BBLImage);
	...
	while(1){
		...
		//	SetCursorPos(LCP.x, LCP.y);//マウスの座標に赤の重心の座標を代入
			...
			if((LCP.x == 0 && RCP.x == 0) || (LCP.y == 0 && RCP.y == 0)){
				printf("触れていません\n");
			} else if(abs(LCP.x - RCP.x) <= 80 &&  abs(LCP.y - RCP.y) <= 50){
				printf("触れています\n");
			...
			while(i>0){
			if(abs(LCP.x - RCP.x) <= 50 &&  abs(LCP.y - RCP.y) <= 50){
			  GetCursorPos(&LSP);//マウスの座標取得
		...
		circle(Result, RC, RADIUS, CV_RGB(255, 170, 0), 2, CV_AA, 0);
	...
	bitwise_and(Result,BBRImage,Result);
	bitwise_or(Result,BGLImage,Result);
	imshow("Result", Result);

		imshow("LresultImage", LdstImage);
		imshow("RresultImage", RdstImage);
    //	imshow("BGRIGmage",BGRImage);作業確認用
    // imshow("BBRIGmage",BBRImage);作業確認用
		 
    //	imshow("BGLGmage",BGLImage);作業確認用
    // imshow("BBLGmage",BBLImage);作業確認用
