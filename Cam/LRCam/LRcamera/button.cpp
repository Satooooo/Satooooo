...
{	
	...
	HWND windowHandle = ::FindWindowA(NULL, "screen");
	SetWindowLongPtr(windowHandle,  GWL_STYLE, WS_MAXIMIZE);
	//SetWindowPos(windowHandle, HWND_TOPMOST, 1280, 0, SCREENWIDTH, SCREENHEIGHT, SWP_DRAWFRAME | SWP_SHOWWINDOW | SWP_FRAMECHANGED);
	IplImage* screen_img = NULL;
	cvShowImage("screen", screen_img);

	//モードボタンの描画
	namedWindow("mode_button", CV_WINDOW_AUTOSIZE);
	windowHandle = ::FindWindowA(NULL, "mode_button");
	// ウィンドウスタイル変更（メニューバーなし、最前面）
	SetWindowLongPtr(windowHandle,  GWL_STYLE, WS_POPUP);
	SetWindowPos(windowHandle, HWND_TOPMOST, 1280+50, 80, 50, 50, SWP_DRAWFRAME | SWP_SHOWWINDOW | SWP_FRAMECHANGED);
	...
	SetWindowLongPtr(windowHandle,  GWL_STYLE, WS_POPUP);
	SetWindowPos(windowHandle, HWND_TOPMOST, 1280+50, 200, 50, 50, SWP_DRAWFRAME | SWP_SHOWWINDOW | SWP_FRAMECHANGED);
	...
	namedWindow("screen2",0);
	cvResizeWindow("screen2",SCREENWIDTH, SCREENHEIGHT);

	Mat BGLImage =cv::Mat::zeros(480, 640, CV_8UC3);//描画用 背景を0に指定
	Mat BBLImage =cv::Mat::zeros(480,640,CV_8UC3);
	bitwise_not(BBLImage, BBLImage);

	Mat BGRImage =cv::Mat::zeros(480, 640, CV_8UC3);//描画用 背景を0に指定
	Mat BBRImage =cv::Mat::zeros(480,640,CV_8UC3);
	bitwise_not(BBRImage, BBRImage);

	Mat screen_mat =cv::Mat::zeros(480, 640, CV_8UC3);
	bitwise_not(screen_mat, screen_mat);
	...
	while(1){
		...
		if(mouseflag==1){//aを押したとき
			...
			if((LCP.x == 0 && RCP.x == 0) || (LCP.y == 0 && RCP.y == 0)){
				printf("触れていません\n");
			} else if(abs(LCP.x - RCP.x) <= 60 &&  abs(LCP.y - RCP.y) <= 40){
				printf("触れています\n");
				//if (get_current_mode() == DRAWING_MODE){
					int Rx[100],Ry[100],Lx[100],Ly[100],i=100;
					while(i>0){
						Rx[i]=RC.x;
						Ry[i]=RC.y;
						Lx[i]=LC.x;
						...
						if(Lx[i]>0 && Lx[i-1]>0){
							line(BGLImage,cv::Point(Lx[i],Ly[i]),cv::Point(Lx[i-1],Ly[i-1]),cv::Scalar(0,255,0),8,CV_AA);
							line(BBLImage,cv::Point(Lx[i],Ly[i]),cv::Point(Lx[i-1],Ly[i-1]),cv::Scalar(0,255,0),8,CV_AA);
						}
						if(Rx[i]>0 && Rx[i-1]>0){
							line(BGRImage,cv::Point(Rx[i],Ry[i]),cv::Point(Rx[i-1],Ry[i-1]),cv::Scalar(0,255,0),8,CV_AA);
							line(BBRImage,cv::Point(Rx[i],Ry[i]),cv::Point(Rx[i-1],Ry[i-1]),cv::Scalar(0,255,0),8,CV_AA);
						}
					}
				//}
		...
		bitwise_and(screen_mat,BBRImage,screen_mat);
		bitwise_or(screen_mat,BGRImage,screen_mat);

		imshow("LresultImage", LdstImage);
		imshow("RresultImage", RdstImage);

		imshow("screen2", screen_mat);
	...
	cvReleaseImage(&mode_img);
	cvReleaseImage(&ss_img);
